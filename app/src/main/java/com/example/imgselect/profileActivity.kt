...
class ProfileData(context: Context) {
    ...
    private val keyInterests = "interests"
    ...
    fun addInterest(interest: String) {
        val interests = loadInterests().toMutableList()
    ...
    fun getWebsitesByGenre(genre: String): List<Website> {
        return Website.websites.filter { it.genreName == genre }
    }

    fun getWebsitesByInterests(): List<Website> {
        val userInterests = getInterests()
        val filteredWebsites = mutableListOf<Website>()

        for (interest in userInterests) {
            val websitesByInterest = Website.websites.filter { it.genreName == interest }
            filteredWebsites.addAll(websitesByInterest)
        }

        return filteredWebsites
    }
    private fun saveInterests(interests: List<String>) {
        val json = Gson().toJson(interests)
        editor.putString(keyInterests, json)
        editor.commit()
    }

    private fun loadInterests(): List<String> {
        val json = preferences.getString(keyInterests, null)
        return if (json != null) {
            Gson().fromJson(json, object : TypeToken<List<String>>() {}.type)
        } else {
            emptyList() // Return an empty list if no interests are found
        }
    }
